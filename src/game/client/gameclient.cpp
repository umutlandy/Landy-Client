...
void CGameClient::OnPredict()
{
	...
	// Landy: compute prediction ticks based on active mode
	const bool LandyUseAmadeus = g_Config.m_TcFastInput && g_Config.m_LandyFastInputMode == 2;
	const bool LandyUseSasuke = g_Config.m_TcFastInput && g_Config.m_LandyFastInputMode == 3;

	int FastInputTicks = 0;
	int FastInputOtherTicks = 0;
	if(LandyUseAmadeus)
	{
		// Amadeus+: ceil(amount/100) ticks for self, slightly reduced for others
		float SelfOffset = std::clamp(g_Config.m_LandyAmadeusAmount, 100, 500) / 100.0f;
		FastInputTicks = (int)std::ceil(SelfOffset);
		if(g_Config.m_LandyAmadeusOthers)
		{
			float OtherOffset = std::min(SelfOffset, std::max(1.35f, SelfOffset * 0.85f));
			FastInputOtherTicks = (int)std::ceil(OtherOffset);
		}
	}
	else if(LandyUseSasuke)
	{
		// Sasuke+: ceil(amount/100) for movement, ceil(max(1.35, amount/100)) for action
		float SelfOffset = std::clamp(g_Config.m_LandySasukeAmount, 100, 500) / 100.0f;
		int MovementTicks = (int)std::ceil(SelfOffset);
		int ActionTicks = (int)std::ceil(std::max(1.35f, SelfOffset));
		FastInputTicks = std::max(MovementTicks, ActionTicks);
		if(g_Config.m_LandySasukeOthers)
		{
			float OtherOffset = std::min(SelfOffset, std::max(1.35f, SelfOffset * 0.85f));
			FastInputOtherTicks = (int)std::ceil(OtherOffset);
		}
	...
	// Sasuke+ brake priority: on direction reverse/release, boost movement ticks for 1 tick
	static int s_aLandySasukeLastDir[2] = {0, 0};
	static int s_aLandySasukeBrakeUntil[2] = {-1, -1};
	...
	if(LandyUseSasuke)
	{
		...
		const int PrevDir = s_aLandySasukeLastDir[LocalTee];
		const bool ReverseEdge = PrevDir != 0 && Direction == -PrevDir;
		const bool ReleaseEdge = PrevDir != 0 && Direction == 0;
		if(ReverseEdge || ReleaseEdge)
			s_aLandySasukeBrakeUntil[LocalTee] = Client()->GameTick(g_Config.m_ClDummy) + 1;
		s_aLandySasukeLastDir[LocalTee] = Direction;
		if(Client()->GameTick(g_Config.m_ClDummy) <= s_aLandySasukeBrakeUntil[LocalTee])
		{
			// 30 ms brake amount → ceil((30+19)/20) = 2 ticks
			const int BrakeTicks = (30 + 19) / 20;
			FastInputTicks = std::max(FastInputTicks, BrakeTicks);
		}
	}
	else
	{
		s_aLandySasukeLastDir[LocalTee] = std::clamp(m_Controls.m_aFastInput[LocalTee].m_Direction, -1, 1);
		s_aLandySasukeBrakeUntil[LocalTee] = -1;
	}
	...
	for(int Tick = Client()->GameTick(g_Config.m_ClDummy) + 1; Tick <= FinalTickSelf; Tick++)
	{
		...
		CNetObj_PlayerInput DummyFastInputDummy{};
		bool DummyFirst = pInputData && pDummyInputData && pDummyChar->GetCid() < pLocalChar->GetCid();

		const bool FastInputActive = g_Config.m_TcFastInput || LandyUseAmadeus || LandyUseSasuke;
		if(FastInputActive && Tick > FinalTickRegular)
		{
			...
			{
				// Composed input for Sasuke+ (Zenish+ port)
				float SelfOffset = std::clamp(g_Config.m_LandySasukeAmount, 100, 500) / 100.0f;
				int MovementTicks = (int)std::ceil(SelfOffset);
				int ActionTicks = (int)std::ceil(std::max(1.35f, SelfOffset));
				...
				if(Client()->GameTick(g_Config.m_ClDummy) <= s_aLandySasukeBrakeUntil[LocalTee])
				{
					const int BrakeAmountMs = 30;
					int BrakeTicks = (BrakeAmountMs + 19) / 20;
					MovementTicks = std::max(MovementTicks, BrakeTicks);
				}

				const CNetObj_PlayerInput BaseInput = pInputData ? *pInputData : m_Controls.m_aFastInput[LocalTee];
				DummyFastInput = BaseInput;
				int OverrunTicks = Tick - FinalTickRegular;

				if(OverrunTicks <= MovementTicks)
				{
					DummyFastInput.m_Direction = m_Controls.m_aFastInput[LocalTee].m_Direction;
					DummyFastInput.m_Jump = m_Controls.m_aFastInput[LocalTee].m_Jump;
					DummyFastInput.m_PlayerFlags = m_Controls.m_aFastInput[LocalTee].m_PlayerFlags;
				}
				if(OverrunTicks <= ActionTicks)
				{
					DummyFastInput.m_Fire = m_Controls.m_aFastInput[LocalTee].m_Fire;
					DummyFastInput.m_Hook = m_Controls.m_aFastInput[LocalTee].m_Hook;
					DummyFastInput.m_TargetX = m_Controls.m_aFastInput[LocalTee].m_TargetX;
					DummyFastInput.m_TargetY = m_Controls.m_aFastInput[LocalTee].m_TargetY;
					DummyFastInput.m_WantedWeapon = m_Controls.m_aFastInput[LocalTee].m_WantedWeapon;
					DummyFastInput.m_NextWeapon = m_Controls.m_aFastInput[LocalTee].m_NextWeapon;
					DummyFastInput.m_PrevWeapon = m_Controls.m_aFastInput[LocalTee].m_PrevWeapon;
				}
				pInputData = &DummyFastInput;
			}
			else
			{
				pInputData = &m_Controls.m_aFastInput[LocalTee];
			...
			if(GetDummyFastInput(DummyFastInputDummy, pDummyInputData, pDummyChar, LocalTee, DummyTee))
				pDummyInputData = &DummyFastInputDummy;
		}
...
void CGameClient::CClientData::Reset()
{
	...
	for(auto &Info : m_aSixup)
		Info.Reset();
	ResetLandyRenderState();
}

void CGameClient::CClientData::ResetLandyRenderState()
{
	m_LandyFastRenderPos = vec2(0.0f, 0.0f);
	m_LandyFastRenderPosValid = false;
	m_LandyFastInteractionState = 0;
	m_LandyFastSaveWindow = 0.0f;
	m_LandyFastLastFreezeTime = 0;
	m_LandyFastLastVel = vec2(0.0f, 0.0f);
	m_LandyFastLastVelValid = false;
}
...
void CGameClient::UpdateRenderedCharacters()
{
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		...
		if(Predict() && (i == m_Snap.m_LocalClientId || (AntiPingPlayers() && !IsOtherTeam(i))) && pChar)
		{
			...
			const bool LandyFastInputActive = g_Config.m_TcFastInput;
			if(g_Config.m_TcRemoveAnti)
				Pos = GetFreezePos(i);
			else if(LandyFastInputActive && (i == m_Snap.m_LocalClientId || (PredictDummy() && i == m_aLocalIds[!g_Config.m_ClDummy])))
			...
			else
			{
				...
				const bool LandyOthersActive = (g_Config.m_LandyFastInputMode < 2 && g_Config.m_TcFastInput && g_Config.m_TcFastInputOthers) ||
											   (g_Config.m_LandyFastInputMode == 2 && g_Config.m_TcFastInput && g_Config.m_LandyAmadeusOthers) ||
											   (g_Config.m_LandyFastInputMode == 3 && g_Config.m_TcFastInput && g_Config.m_LandySasukeOthers);
				if(g_Config.m_TcRemoveAnti && m_pClient->m_IsLocalFrozen)
					Pos = GetFreezePos(i);
				else if(LandyOthersActive && !g_Config.m_TcAntiPingImproved)
					Pos = GetFastInputPos(i);
...
float CGameClient::GetLandyFastInputOffsetTicks(int ClientId) const
{
	// Each input system is fully independent — no master switch required
	const bool UseAmadeus = g_Config.m_TcFastInput && g_Config.m_LandyFastInputMode == 2;
	const bool UseSasuke = g_Config.m_TcFastInput && g_Config.m_LandyFastInputMode == 3;

	const bool IsLocal = ClientId == m_Snap.m_LocalClientId ||
		(PredictDummy() && ClientId == m_aLocalIds[!g_Config.m_ClDummy]);
	...
	if(UseAmadeus)
	{
		...
		if(IsLocal)
			...
			return g_Config.m_LandyAmadeusOthers ? std::min(SelfOffset, std::max(1.35f, SelfOffset * 0.85f)) : 0.0f;
	}
	else if(UseSasuke)
	{
		float SelfOffset = std::clamp(g_Config.m_LandySasukeAmount, 100, 500) / 100.0f;
		if(IsLocal)
			...
			return g_Config.m_LandySasukeOthers ? std::min(SelfOffset, std::max(1.35f, SelfOffset * 0.85f)) : 0.0f;
	}
	else if(g_Config.m_TcFastInput)
	{
		if(IsLocal)
			...
			return g_Config.m_TcFastInputOthers ? g_Config.m_TcFastInputAmount / 20.0f : 0.0f;
	}
	return 0.0f;
}

int CGameClient::GetLandyFastInputTicks(int ClientId, float OffsetTicks) const
{
	const bool UseAmadeus = g_Config.m_TcFastInput && g_Config.m_LandyFastInputMode == 2;
	const bool UseSasuke = g_Config.m_TcFastInput && g_Config.m_LandyFastInputMode == 3;
	...
	else if(UseSasuke)
	{
		...
		{
			float SelfOffset = std::clamp(g_Config.m_LandySasukeAmount, 100, 500) / 100.0f;
			int FastInputMovementTicks = (int)std::ceil(SelfOffset);
			int FastInputActionTicks = (int)std::ceil(std::max(1.35f, SelfOffset));

			static int s_aLandySasukeLastDirection[2] = {0, 0};
			static int s_aLandySasukeBrakeUntilTick[2] = {-1, -1};
			int LocalTee = g_Config.m_ClDummy ^ m_IsDummySwapping;

			if(Client()->GameTick(g_Config.m_ClDummy) <= s_aLandySasukeBrakeUntilTick[LocalTee])
			{
				const int BrakeAmountMs = 30;
				int BrakeTicks = (BrakeAmountMs + 19) / 20;
				FastInputMovementTicks = std::max(FastInputMovementTicks, BrakeTicks);
			}

			return std::max(FastInputMovementTicks, FastInputActionTicks);
		}
		else
		{
			return (int)std::ceil(OffsetTicks);
	...
	{
		return (g_Config.m_TcFastInputAmount + 19) / 20;
	}
	return 0;
}

static void LandyApplyFastInputOffset(float OffsetTicks, int &Tick, float &Intra)
{
	if(OffsetTicks <= 0.0f)
		return;
...
{
	float FastInputOffsetTicks = GetLandyFastInputOffsetTicks(ClientId);
	int FastInputTicks = GetLandyFastInputTicks(ClientId, FastInputOffsetTicks);
	...
	for(int i = 0; i < 2; i++)
	{
		...
		if(in_range(TimePassed, (int64_t)0, Len - 1))
		{
			...
			{
				if(UseAmadeus || UseSasuke)
				{
					if(FastInputOffsetTicks > 0.0f)
						LandyApplyFastInputOffset(FastInputOffsetTicks, SmoothTick, SmoothIntra);
				}
				else
				{
					if(g_Config.m_TcFastInputOthers && FastInputTicks > 0)
...
vec2 CGameClient::GetFastInputPos(int ClientId)
{
	...
	if(UseAmadeus || UseSasuke)
	{
		LandyApplyFastInputOffset(FastInputOffsetTicks, PredTick, PredIntraTick);
		if(PredTick > 0 &&
	...
	else if(g_Config.m_TcFastInput)
	{
		...
		int FastInputTicksVanilla = g_Config.m_TcFastInputAmount / 20;
		...
		FastInputTicksVanilla += CarryOverTicks;

		int FinalTick = PredTick + FastInputTicksVanilla;

		if(FinalTick > 0 &&
			m_aClients[ClientId].m_aPredTick[(FinalTick - 1) % 200] >= Client()->PrevGameTick(g_Config.m_ClDummy) &&
			m_aClients[ClientId].m_aPredTick[FinalTick % 200] <= Client()->PredGameTick(g_Config.m_ClDummy) + FastInputTicksVanilla)
		{
			Pos = mix(m_aClients[ClientId].m_aPredPos[(FinalTick - 1) % 200], m_aClients[ClientId].m_aPredPos[FinalTick % 200], FinalIntra);
		}
	}

	if(IsLocal && (UseAmadeus || UseSasuke))
	{
		CClientData &ClientData = m_aClients[ClientId];
		int FastInputSlot = g_Config.m_ClDummy ^ m_IsDummySwapping;
		if(ClientId == m_aLocalIds[0])
			FastInputSlot = 0;
		else if(ClientId == m_aLocalIds[1])
			FastInputSlot = 1;

		static int s_aLandySasukeLastDirection[2] = {0, 0};
		static int s_aLandySasukeBrakeUntilTick[2] = {-1, -1};
		const bool SasukeBrakeWindow = UseSasuke && FastInputSlot >= 0 && FastInputSlot < 2 && Client()->GameTick(g_Config.m_ClDummy) <= s_aLandySasukeBrakeUntilTick[FastInputSlot];

		const float Dt = std::clamp(Client()->RenderFrameTime(), 0.0f, 0.1f);
		const int SharpnessConfig = UseAmadeus ? g_Config.m_LandyAmadeusCorrection : g_Config.m_LandySasukeCorrection;
		const float Sharpness = std::clamp(SharpnessConfig / 100.0f, 0.0f, 1.0f);
		
		float InteractionStrength = UseAmadeus ? 0.5f : 0.55f;
		
		const float Error = ClientData.m_LandyFastRenderPosValid ? distance(ClientData.m_LandyFastRenderPos, Pos) : 0.0f;
		...
		const float VelImpulse = (pChar && ClientData.m_LandyFastLastVelValid) ? distance(CurrentVel, ClientData.m_LandyFastLastVel) : 0.0f;

		if(ClientData.m_LandyFastRenderPosValid)
		{
			const bool JustSavedFromFreeze = ClientData.m_LandyFastLastFreezeTime > 0 && FreezeTime == 0 && Error > 36.0f && Error < 240.0f;
			...
			if(JustSavedFromFreeze || HammerLikeImpulse || StrongInteractionImpulse)
				ClientData.m_LandyFastSaveWindow = std::max(ClientData.m_LandyFastSaveWindow, mix(0.24f, 0.58f, InteractionStrength));
			else
				ClientData.m_LandyFastSaveWindow = std::max(0.0f, ClientData.m_LandyFastSaveWindow - Dt);
		}
		else
			ClientData.m_LandyFastSaveWindow = 0.0f;

		const bool FreezeSaveWindow = ClientData.m_LandyFastSaveWindow > 0.0f;

		float SnapThreshold = UseAmadeus ? 132.0f : 210.0f;
		if(FreezeSaveWindow)
			SnapThreshold = mix(220.0f, UseSasuke ? 430.0f : 380.0f, InteractionStrength);
		else if(HookingPlayer || DraggedByPlayer)
			SnapThreshold = mix(140.0f, UseSasuke ? 285.0f : 240.0f, InteractionStrength);

		const bool Snap = !ClientData.m_LandyFastRenderPosValid || Error > SnapThreshold;
		if(Snap)
		{
			ClientData.m_LandyFastRenderPos = Pos;
			ClientData.m_LandyFastInteractionState = 4; // SNAP
		}
		else
		{
			float Follow = 0.0f;
			if(UseAmadeus)
				Follow = 0.30f + Sharpness * 0.48f + std::clamp(Error / 150.0f, 0.0f, 0.22f);
			else // Sasuke+
			{
				Follow = 0.10f + Sharpness * 0.34f + std::clamp(Error / 215.0f, 0.0f, 0.18f);
				if(SasukeBrakeWindow)
					Follow = std::max(Follow, 0.48f + Sharpness * 0.30f + std::clamp(Error / 120.0f, 0.0f, 0.18f));
			}

			if(FreezeSaveWindow)
			{
				Follow = mix(0.22f, 0.16f, InteractionStrength) + Sharpness * mix(0.36f, 0.26f, InteractionStrength) + std::clamp(Error / 280.0f, 0.0f, mix(0.28f, 0.20f, InteractionStrength));
				ClientData.m_LandyFastInteractionState = 3; // FREEZE_SAVE
			}
			else if(DraggedByPlayer)
			{
				Follow = mix(0.36f, 0.28f, InteractionStrength) + Sharpness * mix(0.42f, 0.32f, InteractionStrength) + std::clamp(Error / 185.0f, 0.0f, mix(0.18f, 0.12f, InteractionStrength));
				ClientData.m_LandyFastInteractionState = 2; // DRAGGED;
			}
			else if(HookingPlayer)
			{
				Follow = mix(0.38f, 0.30f, InteractionStrength) + Sharpness * mix(0.44f, 0.34f, InteractionStrength) + std::clamp(Error / 185.0f, 0.0f, mix(0.18f, 0.12f, InteractionStrength));
				ClientData.m_LandyFastInteractionState = 1; // HOOK
			}
			else
				ClientData.m_LandyFastInteractionState = 0; // NONE

			const float MinFollow = UseSasuke ? (SasukeBrakeWindow ? 0.42f : 0.075f) : 0.28f;
			Follow = std::clamp(Follow, MinFollow, 1.0f);
			ClientData.m_LandyFastRenderPos = mix(ClientData.m_LandyFastRenderPos, Pos, Follow);
		}

		ClientData.m_LandyFastLastFreezeTime = FreezeTime;
		ClientData.m_LandyFastLastVel = CurrentVel;
		ClientData.m_LandyFastLastVelValid = pChar != nullptr;
		ClientData.m_LandyFastRenderPosValid = true;
		Pos = ClientData.m_LandyFastRenderPos;
	}
...
vec2 CGameClient::GetFreezePos(int ClientId)
{
	...
	const bool IsLocal = ClientId == m_Snap.m_LocalClientId || (PredictDummy() && ClientId == m_aLocalIds[!g_Config.m_ClDummy]);
	const bool LandyUseAmadeusFreeze = g_Config.m_TcFastInput && g_Config.m_LandyFastInputMode == 2;
	const bool LandyUseSasukeFreeze = g_Config.m_TcFastInput && g_Config.m_LandyFastInputMode == 3;
	...
	if(IsLocal)
	{
		if(LandyUseAmadeusFreeze || LandyUseSasukeFreeze)
		{
			LandyApplyFastInputOffset(FastInputOffsetTicks, SmoothTick, SmoothIntra);
		}
		else if(g_Config.m_TcFastInput)
		{
			...
			int CarryOverTicks = static_cast<int>(IntraRemainder);
			FastInputTicksVanilla += CarryOverTicks;
			FastInputTicks = FastInputTicksVanilla;
			SmoothTick += FastInputTicksVanilla;
			SmoothIntra = FinalIntra;
		}
	}
	else // not local
	{
		if((LandyUseAmadeusFreeze && g_Config.m_LandyAmadeusOthers) || (LandyUseSasukeFreeze && g_Config.m_LandySasukeOthers))
		{
			LandyApplyFastInputOffset(FastInputOffsetTicks, SmoothTick, SmoothIntra);
		}
		else if(!LandyUseAmadeusFreeze && !LandyUseSasukeFreeze && g_Config.m_TcFastInput && g_Config.m_TcFastInputOthers)
		{
			float FastInputIntra = (g_Config.m_TcFastInputAmount % 20) / 20.0f;
			int FastInputTicksVanilla = g_Config.m_TcFastInputAmount / 20;
			float CombinedIntra = SmoothIntra + FastInputIntra;
