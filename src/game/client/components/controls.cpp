...
int CControls::SnapInput(int *pData)
{
	...
	else
	{
		...
		// Landy Input keeps the existing precision direction handling. It is only
		// active while the Landy mode itself is selected and enabled.
		if(g_Config.m_TcFastInput && g_Config.m_LandyFastInputMode == 1 && g_Config.m_LandyPrecisionInput)
		{
			// Update last-pressed tracker when only a single key is held
			if(m_aInputDirectionLeft[g_Config.m_ClDummy] && !m_aInputDirectionRight[g_Config.m_ClDummy])
				m_aLastPressedDirection[g_Config.m_ClDummy] = -1;
			else if(!m_aInputDirectionLeft[g_Config.m_ClDummy] && m_aInputDirectionRight[g_Config.m_ClDummy])
				m_aLastPressedDirection[g_Config.m_ClDummy] = 1;
			else if(!m_aInputDirectionLeft[g_Config.m_ClDummy] && !m_aInputDirectionRight[g_Config.m_ClDummy])
				m_aLastPressedDirection[g_Config.m_ClDummy] = 0; // neither key held — reset

			// When both keys are held simultaneously, resolve using last pressed direction
			// instead of cancelling out to 0. This avoids unintended stalls in tight one-tiles.
			if(m_aInputDirectionLeft[g_Config.m_ClDummy] && m_aInputDirectionRight[g_Config.m_ClDummy])
				m_aInputData[g_Config.m_ClDummy].m_Direction = m_aLastPressedDirection[g_Config.m_ClDummy];
		}
		else
		{
			// Precision input disabled — reset tracker so it's clean when re-enabled
			m_aLastPressedDirection[g_Config.m_ClDummy] = m_aInputData[g_Config.m_ClDummy].m_Direction;
		}
