...
void CChat::OnRender()
{
	...
	if(m_Mode != MODE_NONE)
	{
		...
		{
			const float TranslateButtonSize = std::max(16.0f, ScaledFontSize * 1.35f);
...
void CChat::SendChatQueued(const char *pLine)
{
	...
	// TClient: try to translate outgoing chat before sending
	const int Team = m_Mode == MODE_ALL ? 0 : 1;
	if(GameClient()->m_Translate.TryTranslateOutgoingChat(Team, pLine))
		return; // Translation is pending; SendTranslatedChatQueued will be called when done
