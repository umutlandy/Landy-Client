...
{
	m_Screen.h = 600.0f;
	// UI always uses the native window aspect. The custom aspect ratio is a
	// game render-target setting and must not alter menu/text layout.
	m_Screen.w = Graphics()->ScreenWidth() / (float)Graphics()->ScreenHeight() * m_Screen.h;
	return &m_Screen;
}
