...
{
	m_WindowViewportWidth = pCommand->m_Width;
	m_WindowViewportHeight = pCommand->m_Height;
...
void CCommandProcessorFragment_OpenGL::DestroyCustomRenderTarget()
{
	if(m_CustomRenderTexture)
		glDeleteTextures(1, &m_CustomRenderTexture);
	if(m_CustomRenderFbo)
		glDeleteFramebuffers(1, &m_CustomRenderFbo);
	m_CustomRenderFbo = m_CustomRenderTexture = 0;
	m_CustomRenderWidth = m_CustomRenderHeight = 0;
}

void CCommandProcessorFragment_OpenGL::UpdateCustomRenderTarget()
{
	const bool Enable = g_Config.m_LandyCustomAspectPreset != 0;
	const int Width = g_Config.m_LandyCustomAspectAppliedWidth;
	const int Height = g_Config.m_LandyCustomAspectAppliedHeight;
	if(!Enable)
	{
		if(m_CustomRenderFbo)
			DestroyCustomRenderTarget();
		m_CustomRenderActive = false;
		return;
	}
	if(!m_CustomRenderFbo || m_CustomRenderWidth != Width || m_CustomRenderHeight != Height)
	{
		DestroyCustomRenderTarget();
		glGenFramebuffers(1, &m_CustomRenderFbo);
		glBindFramebuffer(GL_FRAMEBUFFER, m_CustomRenderFbo);
		glGenTextures(1, &m_CustomRenderTexture);
		glBindTexture(GL_TEXTURE_2D, m_CustomRenderTexture);
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, Width, Height, 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
		glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, m_CustomRenderTexture, 0);
		...
		m_CustomRenderWidth = Width;
		m_CustomRenderHeight = Height;
	}
	m_CustomRenderActive = true;
	glBindFramebuffer(GL_FRAMEBUFFER, m_CustomRenderFbo);
	glViewport(0, 0, Width, Height);
}

void CCommandProcessorFragment_OpenGL::OnSwap()
{
	if(!m_CustomRenderActive || !m_CustomRenderFbo || m_WindowViewportWidth <= 0 || m_WindowViewportHeight <= 0)
		return;
	// UI components can leave a scissor rectangle active. The final FBO blit
	// must cover the entire native window, independent of the last UI clip.
	const bool ScissorEnabled = glIsEnabled(GL_SCISSOR_TEST);
	if(ScissorEnabled)
		glDisable(GL_SCISSOR_TEST);
	glBindFramebuffer(GL_READ_FRAMEBUFFER, m_CustomRenderFbo);
	glBindFramebuffer(GL_DRAW_FRAMEBUFFER, 0);
	glBlitFramebuffer(0, 0, m_CustomRenderWidth, m_CustomRenderHeight, 0, 0, m_WindowViewportWidth, m_WindowViewportHeight, GL_COLOR_BUFFER_BIT, GL_LINEAR);
	glBindFramebuffer(GL_FRAMEBUFFER, 0);
	glViewport(0, 0, m_WindowViewportWidth, m_WindowViewportHeight);
	if(ScissorEnabled)
